...
| `browser` | ID numérica do navegador. References the `browser.tsv` lookup table. | int unsigned |
...
| `c_color` | Profundidade de bits da paleta de cores. Used as part of calculating the [Color depth](/help/components/dimensions/color-depth.md) dimension. O AppMeasurement usa a função JavaScript `screen.colorDepth()`. | char(20) |
| `campaign` | Variable used in the [Tracking Code](/help/components/dimensions/tracking-code.md) dimension. | varchar(255) |
| `carrier` | Variável de integração da Adobe Advertising Cloud. Especifica a operadora de celular. References the `carrier` lookup table. | varchar(100) |
...
| `connection_type` | ID numérica que representa o tipo de conexão. Variable used in the [Connection type](/help/components/dimensions/connection-type.md) dimension. References the `connection_type.tsv` lookup table. | tinyint unsigned |
| `cookies` | Variable used in the [Cookie support](/help/components/dimensions/cookie-support.md) dimension.<br>Y: Ativado<br>N: Desativado<br>U: Desconhecido | char(1) |
| `country` | ID numérica que representa o país de onde a ocorrência foi originada. Used in the [Countries](/help/components/dimensions/countries.md) dimension. Usa `country.tsv` pesquisa. | smallint unsigned |
| `ct_connect_type` | Related to the `connection_type` column. Os valores mais comuns são LAN/Wifi, Operadora de celular e Modem. | char(20) |
...
| `domain` | Variable used in the [Domain](/help/components/dimensions/domain.md) dimension. Baseado no ponto de acesso à Internet do visitante. | varchar(100) |
...
| `geo_city` | Nome da cidade na qual a ocorrência foi originada, com base no IP. Used in the [Cities](/help/components/dimensions/cities.md) dimension. | char(32) |
| `geo_country` | Abreviação do país no qual a ocorrência foi originada, com base no IP. | char(4) |
| `geo_dma` | ID numérica da área demográfica na qual a ocorrência foi originada, com base no IP. Usada na dimensão DMA [](/help/components/dimensions/us-dma.md) dos EUA. | int unsigned |
| `geo_region` | Nome do estado ou região de origem da ocorrência, com base no IP. Used in the [Regions](/help/components/dimensions/regions.md) dimension. | char(32) |
...
| `javascript` | Lookup ID of JavaScript version, based on `j_jscript`. Usa uma tabela de pesquisa. | tinyint unsigned |
| `language` | ID numérica da linguagem. Uses `languages.tsv` lookup table. | smallint unsigned |
| `last_hit_time_gmt` | Carimbo de data e hora (em horário Unix) da ocorrência anterior. Used to calculate the [Days since last visit](/help/components/dimensions/days-since-last-visit.md) dimension. | int |
| `last_purchase_num` | Variable used in the [Customer loyalty](/help/components/dimensions/customer-loyalty.md) dimension. O número de compras anteriores realizadas pelo visitante. <br>0: Nenhuma compra anterior (não é um cliente) <br>1: 1 compra prévia (novo cliente) <br>2: 2 compras anteriores (cliente recorrente) <br>3: 3 ou mais compras anteriores (cliente fidelizado) | int unsigned |
| `last_purchase_time_gmt` | Used in the [Days since last purchase](/help/components/dimensions/days-since-last-purchase.md) dimension. Carimbo de data e hora (em horário Unix) da última compra feita. Para compras feitas pela primeira vez e visitantes que ainda não fizeram uma compra, esse valor é `0`. | int |
...
| `mobileaction` | Ação em dispositivo móvel. Automatically collected when `trackAction` is called in Mobile Services. Permite a criação de caminhos de ação automática no aplicativo. | varchar(100) |
...
| `os` | ID numérica que representa o sistema operacional do visitante. Based on the `user_agent` column. Usa `os` pesquisa. | int unsigned |
| `p_plugins` | Não está mais em uso. Lista de plug-ins disponíveis para o navegador. Used the JavaScript function `navigator.plugins()`. | texto |
...
| `page_url` | O URL da ocorrência. Removidas das solicitações de imagem de rastreamento de link. | varchar(255) |
...
| `persistent_cookie` | Used in the [Persistent cookie support](/help/components/dimensions/persistent-cookie-support.md) dimension. Indica se o visitante suporta cookies que não são descartados depois de cada ocorrência. | char(1) |
| `plugins` | Não está mais em uso. Lista de IDs numéricas que correspondem a plug-ins disponíveis no navegador. Usa `plugins.tsv` pesquisa. | varchar(180) |
...
| `post_` colunas | Contém o valor usado por último em relatórios. Cada coluna de publicação contém valores após a lógica do lado do servidor, regras de processamento e regras VISTA. A Adobe recomenda usar tais colunas na maioria dos casos. | Consulte a respectiva coluna de não-publicação |
| `prev_page` | Não usado. Identificador proprietário do Adobe da página anterior. | int unsigned |
| `product_list` | Product list as passed in through the [`products`](/help/implement/vars/page-vars/products.md) variable. Produtos são delimitados por vírgulas, propriedades de produtos individuais são delimitados por ponto e vírgula. | texto |
| `product_merchandising` | Não usado. Use `product_list` no lugar dela. | texto |
| `prop1` - `prop75` | Variáveis de tráfego personalizadas 1 - 75. Usado em dimensões [Prop](/help/components/dimensions/prop.md) . | varchar(100) |
...
| `ref_domain` | Com base na coluna referrer. O domínio de referência da ocorrência. Used in the [Referring domain](/help/components/dimensions/referring-domain.md) dimension. | varchar(100) |
| `ref_type` | Uma ID numérica que representa o tipo de referência para a ocorrência. Used in the [Referrer type](/help/components/dimensions/referrer-type.md) dimension. <br>1: Dentro do site<br>2: Outros sites da web <br>3: Mecanismos de pesquisa <br>4: Disco rígido <br>5: USENET <br>6: Digitado/Marcado (sem referenciador) <br>7: E-mail <br>8: Sem JavaScript <br>9: Redes sociais | tinyint unsigned |
| `referrer` | URL da página anterior. Used in the [Referrer](/help/components/dimensions/referrer.md) dimension. Observe que, embora o `referrer` use um tipo de dados de varchar(255), o `post_referrer` usa um tipo de dados de varchar(244). | varchar(255) |
| `resolution` | ID numérica que representa a resolução do monitor. Used in the [Monitor resolution](/help/components/dimensions/monitor-resolution.md) dimension. Uses `resolution.tsv` lookup table. | smallint unsigned |
| `s_kwcid` | A ID de palavra-chave usada em integrações da Adobe Advertising Cloud. | varchar(255) |
| `s_resolution` | Valor bruto da resolução da tela. Gathered using the JavaScript function `screen.width x screen.height`. | char(20) |
| `sampled_hit` | Não está mais em uso. Era usada anteriormente para amostragem na Ad Hoc Analysis. | char(1) |
| `search_engine` | ID numérica que representa o Mecanismo de pesquisa que direcionou o visitante ao seu site. Usa `search_engines.tsv` pesquisa. | smallint unsigned |
| `search_page_num` | Used by the [All Search Page Rank](/help/components/dimensions/all-search-page-rank.md) dimension. Indica em qual página de resultados de pesquisa seu site foi exibido antes de o visitante clicar no seu site. | smallint unsigned |
| `secondary_hit` | Sinalizador que monitora ocorrências secundárias. Geralmente se origina de tags de vários conjuntos e regras VISTA que copiam ocorrências. | tinyint unsigned |
...
| `transactionid` | Um identificador exclusivo, em que vários pontos de dados podem ser carregados posteriormente via fontes de dados. Coletada usando a [`transactionID`](/help/implement/vars/page-vars/transactionid.md) variável. | texto |
...
| `va_closer_id` | Numeric ID that identifies the [Last touch channel](/help/components/dimensions/last-touch-channel.md) dimension. A pesquisa desta ID pode ser encontrada no Gerenciador de canais de marketing. | tinyint unsigned |
...
| `va_finder_id` | Numeric ID that identifies the [First touch channel](/help/components/dimensions/first-touch-channel.md) dimension. A pesquisa desta ID pode ser encontrada no Gerenciador de canais de marketing. | tinyint unsigned |
...
| `visit_keywords` | Variable used in the [Search keyword](/help/components/dimensions/search-keyword.md) dimension. Essa coluna usa um limite de caracteres não padrão de varchar(244) para acomodar a lógica back-end usada pelo Adobe. | varchar(244) |
| `visit_num` | Variable used in the [Visit number](/help/components/dimensions/visit-number.md) dimension. Começa em 1, e incrementa a cada início de nova visita por visitante. | int unsigned |
| `visit_page_num` | Variable used in the [Hit depth](/help/components/dimensions/hit-depth.md) dimension. Aumenta em 1 para cada ocorrência gerada pelo usuário. Redefine cada visita. | int unsigned |
| `visit_ref_domain` | Based on the `visit_referrer` column. O primeiro domínio de referência da visita. | varchar(100) |
| `visit_ref_type` | ID numérica que representa o tipo do primeiro referenciador da visita. Uses the `referrer_type.tsv` lookup table. | tinyint unsigned |
| `visit_referrer` | O primeiro referenciador da visita. | varchar(255) |
| `visit_search_engine` | ID numérica do primeiro mecanismo de pesquisa da ocorrência. Usa `search_engines.tsv` pesquisa. | smallint unsigned |
