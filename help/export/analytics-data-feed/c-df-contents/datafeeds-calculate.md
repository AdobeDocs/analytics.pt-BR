...
> [!NOTE] Irregularidades na Internet, irregularidades no sistema ou o uso de IDs de visitante personalizadas raramente podem usar os mesmos `visit_num` valores para diferentes visitas. Use `visit_start_time_gmt` ao contar visitas para garantir que essas visitas sejam contadas.
...
2. Incluir somente linhas nas quais `event_list` contém o evento purchase.
3. Analise a `post_product_list` coluna para extrair todos os dados de preço. A `post_product_list` coluna é formatada da mesma forma que a `s.products` variável.
