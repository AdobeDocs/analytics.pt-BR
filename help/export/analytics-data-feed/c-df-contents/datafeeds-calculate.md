...
keywords: Feed de dados; tarefa; métricas; coluna anterior; coluna posterior; bots; filtragem de data; sequência de evento; common; fórmulas
...
Os bots são excluídos dos feeds de dados de acordo com as [regras de bot](https://marketing.adobe.com/resources/help/en_US/reference/?f=bot_rules) definidas para o conjunto de relatórios.

## Filtragem de dados {#section_3BFF4F7EED1F4FA69EBF12BF98B347E8}

Inclua linhas no intervalo da datas que deseja incluir filtrando o campo `date_time`. `date_time` O campo é legível por humanos (por exemplo `YYYY-MM-DD HH:MM:SS`,) e é ajustado para o fuso horário do conjunto de relatórios. For example, `date_time starts with "2013-12"` includes hits from December 2013.

## Sequência de evento {#section_87B686512EFD4A6CA072165CB28A130A}

The event string in `event_list` and `post_event_list` contains a comma-delimited list of events, which may have a value and/or a unique ID. Recomendamos fazer todo o processamento no `post_event_list`, pois sua duplicata é removida e a lógica já é aplicada para remover eventos duplicados com a mesma ID (consulte [Serialização de eventos](https://marketing.adobe.com/resources/help/en_US/sc/implement/?f=c_event_serialization)).

Para a ID de evento de nomeação de mapeamento, consulte a pesquisa de evento fornecida com o feed de dados.

Para obter mais informações sobre eventos, consulte [Eventos](https://marketing.adobe.com/resources/help/en_US/sc/implement/index.html?f=c_events).
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> 
    ...
    </ol> <p>*Em raras ocasiões, as irregularidades da Internet, do sistema ou o uso das IDs do visitante personalizadas podem resultar em valores duplicados de <code>visit_num</code> para a mesma ID de visitante que não correspondem aos valores de <a href="https://marketing.adobe.com/resources/help/en_US/reference/?f=metrics_visit" format="http" scope="external">visit </a>. Para evitar esse tipo de problema, inclua <code>visit_start_time_gmt</code> ao contar as visitas. </p> </td> 
  </tr> 
  <tr> 
  ...
  <p>Indica uma instância de <code>purchase</code> e <code>event1 </code>. </p> 
   ...
   <td colname="col2"> <p>Para calcular o tempo gasto, você deve agrupar os hits por visita e, em seguida, agrupá-los de acordo com o número do hit dentro da visita. </p> 
    <ol id="ol_946E7CD6005A42EB9A4B79268BF84066"> 
     ...
     <li id="li_2C5AC0477CFC409B8F169079354C8226">Encontre hits nos quais valor que você deseja para monitorar o tempo gasto já está definido. Por exemplo: 
      <code>hit 1: post_ prop 1 = hit red 2: post_ prop 1 = blue </code>
  </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>A conversão de moeda é aplicada ao <code>post_product_list</code> de acordo com as configurações do conjunto de relatórios. Portanto, é recomendado usar essa coluna. </p> 
    <ol id="ol_03D62086EDDE42AD82049830D85FDC69"> 
     ...
     <li id="li_FA1639FEF516419BA1BFDC37B063B346"> <p>O <code>post_product_list</code> usa a mesma sintaxe como <a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/?f=c_products" format="http" scope="external">s.products</a>. Portanto, é possível analisar essa sequência para calcular métricas. Por exemplo: </p> 
      <code>; Treinadores cruzados; 1; 69.95; Meias atlônicas; 10; 29.99 </code>
  <p>Ao analisar a sequência, é possível determinar que 1 par de tênis de trilha foi comprado por US$ 69,95 e que a receita total da compra foi US$ 99,94. </p> </li> 
    </ol> <p>Observação: como o Analytics permite que eventos de moeda com a receita do produto sejam enviados na sequência de eventos, talvez seja necessário contabilizar a receita que não consta na sequência de produtos. Consulte <i>Eventos numéricos/de moeda</i> em <a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/?f=c_events" format="http" scope="external">s.events </a>. </p> </td> 
  </tr> 
 </tbody> 
</table>
