...
+ Identificação do visitante {#id}
   + [Visão geral](id/overview.md)
   + [Utilização da extensão do SDK da Web](id/web-sdk-extension.md)
...
+ Implementar o Analytics com a Experience Platform Edge {#aep-edge}
   ...
   + [Mapeamento de campos de objetos de dados](aep-edge/data-var-mapping.md)
   + [Mapeamento de campos de objetos XDM](aep-edge/xdm-var-mapping.md)
   ...
   + API da Edge Network {#api}
      + [Visão geral da API da Edge Network](aep-edge/api/overview.md)
...
+ Implementar o Analytics usando JavaScript {#js}
   ...
   + {hide-from-toc}[Implementar links para opção de não participação](js/opt-out.md)
