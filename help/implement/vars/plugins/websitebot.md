...
1. Adicione uma Regra de chamada direta que aciona um beacon do Analytics usando `websiteBot` como o identificador. Neste exemplo, ele usa uma chamada `s.tl`:

   ![identificador do siteBot](assets/websitebot.png)

1. Acione as ações Adobe Analytics - Definir variáveis e Adobe Analytics - Enviar beacon na regra de Chamada direta.  Uma maneira de fazer isso é mostrada no exemplo a seguir:
