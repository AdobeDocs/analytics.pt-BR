...
O plug- `getAndPersistValue` in permite armazenar um valor em um cookie que pode ser recuperado posteriormente durante uma visita. Ela desempenha uma função semelhante ao [!UICONTROL Storage duration] recurso no Adobe Experience Platform Launch. A Adobe recomenda usar esse plug-in se você quiser manter automaticamente uma variável do Analytics com o mesmo valor em ocorrências subsequentes após a definição da variável. Este plug-in não é necessário se o recurso do Launch for suficiente ou se você não precisar definir e persistir variáveis com o mesmo valor em ocorrências subsequentes. [!UICONTROL Storage duration] A persistência integrada das eVars não requer o uso desse plug-in, pois essas variáveis persistem no lado do servidor pela Adobe.
