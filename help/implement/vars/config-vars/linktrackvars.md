...
A `s.linkTrackVars` variável é uma string que contém uma lista de variáveis delimitadas por vírgulas que você deseja incluir nas solicitações de imagem de rastreamento de link (`tl()` função). Ambos os critérios a seguir devem ser atendidos para incluir dimensões nas ocorrências de rastreamento de link:
...
* Se essa variável não estiver definida ou definida como uma string vazia, *todas* as variáveis serão incluídas nas solicitações de imagem de rastreamento de link.
