...
>abstract="A granularidade de tempo se refere à frequência com a qual o alerta é verificado."
...
Para obter informações mais detalhadas sobre alertas, consulte [Visão geral dos alertas](intellligent-alerts.md).
...
1. Use qualquer uma das seguintes maneiras para criar um alerta.: 
   ...
   * Abra um projeto no Analysis Workspace, escolha um ou mais itens de linha em uma tabela de forma livre, clique com o botão direito do mouse e selecione **[!UICONTROL Criar alerta a partir da seleção]**. Esta ação preenche automaticamente o [Criador de alertas](alert-builder.md) para criar um alerta com as métricas e filtros corretos.
   * Criar um alerta [a partir do gerenciador de alertas](/help/components/c-alerts/alert-manager.md#create-alerts).

   O construtor de alertas é exibido. Essa interface é familiar à interface para criar segmentos ou métricas calculadas no Analytics.



## Construtor de alertas

A interface do Criador de alertas é semelhante à interface usada para criar segmentos ou métricas calculadas no Customer Journey Analytics:
