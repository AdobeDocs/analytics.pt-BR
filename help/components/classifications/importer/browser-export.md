...
| Filtro de dados | (Opcional) Filtre dados pelos dados da campanha. É possível baixar dados somente de campanhas ativas ou selecionar campanhas que começaram (ou terminaram) em um intervalo de datas específico.<br>**Importante**: essa opção não está disponível para conjuntos de relatórios habilitados para a Nova Arquitetura de Classificação. |
