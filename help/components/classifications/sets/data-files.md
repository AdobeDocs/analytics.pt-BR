...
>[!NOTE]
>
>Apesar de seguir as convenções para Linhas JSON, use a extensão de arquivo `.json` para todos os uploads. O uso da extensão `.jsonl` pode resultar em erros.
...
Alguns exemplos de registros JSON em um arquivo JSON.
...
Alguns exemplos de registros CSV em um arquivo CSV.
...
#### Excluir campos específicos (misturados com atualizações)
...
### Regras de formatação TSV e TAB
...
### Operações de exclusão de TSV e TAB
...
Alguns exemplos de registros delimitados por TSV ou TAB em um arquivo TSV ou TAB.
...
Problemas comuns e soluções ao carregar arquivos:
