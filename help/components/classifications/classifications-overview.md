...
* **Conjuntos de classificações** são os componentes principais para classificar dados. [Os conjuntos de classificações](/help/components/classifications/sets/overview.md) permitem criar e gerenciar classificações em uma única interface simplificada.

* **Classificações herdadas** documenta os métodos de classificação herdados para classificar dados. Esses métodos estão obsoletos e não estarão mais acessíveis após 31 de agosto de 2026.
