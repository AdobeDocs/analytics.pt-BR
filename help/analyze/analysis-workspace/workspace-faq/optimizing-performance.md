...
| Query simultâneos | O número de query que estão sendo solicitados pela sua organização para Adobe ao mesmo tempo. Cada organização tem direito a um mínimo de cinco query simultâneos. | Se um relatório estiver demorando muito, normalmente é devido ao fato de estar em uma fila com outros relatórios. Isso significa que sua organização está tentando executar várias solicitações simultâneas em relação a um conjunto de relatórios específico. Os query podem ser provenientes de solicitações de API, interfaces do relatórios (Analysis Workspace, Relatórios e análises, Report Builder etc.), projetos agendados, relatórios agendados, alertas agendados e usuários simultâneos que fazem solicitações de relatórios. | Espalhe suas solicitações e agendamentos no conjunto de relatórios de forma mais uniforme ao longo do dia. Além disso, alterne suas solicitações para horários fora de pico quando possível. As manhãs de segunda-feira, terça-feira de manhã e o primeiro de cada mês são horas de pico de relatórios. |
| Capacidade do centro de dados | A quantidade de capacidade do relatórios que você e outros clientes compartilham em um data center do Adobe. | Isso é afetado pelo número de query simultâneos feitos pela sua organização e outras organizações dentro do seu data center. | Sua organização tem direito a uma capacidade definida e, se o sistema estiver sob uma carga leve, o Adobe mudará mais capacidade para você, acima e além da sua permissão autorizada. |
