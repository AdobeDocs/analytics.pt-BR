...
A Análise de contribuição realiza isso executando um algoritmo de duas partes em cada item de dimensão disponível no relatório Análise de contribuição do usuário. O algoritmo opera nesta ordem:

1. Para cada dimensão, calcula a estatística do teste V de Cramer. No seguinte exemplo, considere uma tabela de contingência com exibições de página por país ao longo de dois períodos:

   ![](assets/contingency_table.png)

   Na Tabela 1, o V de Cramer pode ser usado para medir a associação entre as exibições de página por países para o período 1 (por exemplo, histórico) e o período 2 (por exemplo, o dia em que a anomalia ocorreu). Um valor baixo para o V de Cramer implica um baixo nível de associação. O intervalo V de Cramer varia de 0 (sem associação) a 1 (associação completa). A estatística V de Cramer pode ser calculada:
...
1. Para cada item de dimensão, o Residual da Pessoa (PR) é usado para medir a associação entre a métrica anômala e cada item de dimensão. O PR segue uma distribuição normal padrão, que permite ao algoritmo comparar PRs de duas variáveis aleatórias, mesmo que os desvios não sejam comparáveis. Na prática, o erro não é conhecido e é estimado usando correção de amostra finita.
   ...
   Para os resultados finais, a pontuação de cada item de dimensão é ponderada pela medida V de Cramer e redimensionada para um número entre 0 e 1 para fornecer sua pontuação de contribuição.
