...
A página de aterrissagem do Adobe Analytics reúne o Analysis Workspace e o Reports &amp; Analytics em uma única interface e ponto de acesso sob o comando do Workspace. Ela apresenta uma página inicial do gerente de projeto, um menu de relatórios atualizado com relatórios modernizados, bem como uma nova seção de aprendizado que ajuda você a começar de forma mais eficaz.

>[!IMPORTANT]
>
>Essa página de aterrisagem estará na Open Beta a partir de 21 de julho de 2021. É um ponto de partida opcional. Se preferir, você poderá usar sua página de aterrissagem existente: Workspace ou Reports. Se você decidir aderir ao programa beta, acesse a página e clique em **[!UICONTROL Fornecer feedback]** na parte superior da tela para registrar seu feedback e os bugs que encontrar. Você pode recusar e voltar ao programa beta a qualquer momento. Qualquer trabalho feito na interface do usuário beta é transferido para a experiência existentel/atual do Workspace.
...
| --- | --- | --- |
| Expandir [!UICONTROL Projetos] tabela para tela cheia | Para expandir a propriedade da tabela, clique no ícone do menu de hambúrguer. Essa ação recolherá as guias do painel esquerdo. | ![Expandir tabela](assets/landing-collapse.png) |
| Personalizar a largura da coluna | Anteriormente, a largura da coluna era fixa. Agora é possível ajustá-lo arrastando o separador de colunas. |  |
| Reordenar os itens fixados | Para mover os itens fixados para cima e para baixo, clique nas reticências ao lado do item fixado e selecione[!UICONTROL  Mover para cima] ou [!UICONTROL Mover para baixo]. | ![Mover itens fixos](assets/move-up-down.png) |
| Novas colunas da tabela | Clique no botão [!UICONTROL Personalizar tabela] ícone na parte superior direita da tabela. As novas colunas da tabela incluem: <ul><li>**[!UICONTROL Programado]**: Defina como [!UICONTROL Ligado] quando um projeto estiver programado ou [!UICONTROL Desligado] quando não for. Clicar no [!UICONTROL Ligado] permite ver informações sobre o projeto agendado. Você também pode [editar o agendamento do projeto](/help/analyze/analysis-workspace/curate-share/t-schedule-report.md) se você for o proprietário do projeto.</li><li>**[!UICONTROL ID do projeto]**: A ID do projeto pode ser usada para depurar projetos.</li><li>**[!UICONTROL Intervalo de datas mais longo]**: Intervalos de datas mais longos aumentam a complexidade do projeto e podem aumentar os tempos de processamento e carregamento. </li><li>**[!UICONTROL Número de Consultas]**: Um número maior de consultas de projeto aumenta a complexidade do projeto e pode aumentar os tempos de processamento e carregamento.</li></ul> | ![Novas colunas](assets/new-columns.png) |
| Clique uma vez para abrir um relatório | Anteriormente, você tinha que clicar duas vezes. |  |
| Novos links para relatórios do Reports &amp; Analytics | <ul><li>Relatórios > Público-alvo > Bots</li><li>Relatórios > Público-alvo > Páginas de bot<li>Relatórios > Envolvimento > Tempo real</li></ul> | ![Novos links](assets/report-links.png) |
| Novos relatórios prontos para uso | <ul><li>Relatórios > Mais popular > Próxima página</li><li>Relatórios > Mais popular > Página anterior</li><li>Relatórios > Envolvimento > Análise da página > Resumo da página</li></ul>Observe que esses relatórios estão no formato Workspace e exigem configuração e criação. A saída consiste em um painel de métricas de alto nível, dados de tendências, [!UICONTROL Fluxo] visualização e muito mais. Também é possível modificar esses relatórios e alterar dimensões, itens de dimensão etc. Esses relatórios também estão disponíveis como painéis em painéis do Workspace. | ![Próxima página](assets/next-page.png) |
| [!UICONTROL Criar novo] modal está de volta | Ao clicar em [!UICONTROL Criar novo] no Workspace, você mais uma vez obtém a escolha entre um projeto em branco e um scorecard móvel em branco. Você também pode escolher a partir de qualquer modelo criado por sua empresa. | ![Criar novo](assets/create-new.png) |
| Também disponível no CJA | Essa landing page, em um formulário modificado, também estará disponível no CJA. |  |

{style=&quot;table-layout:auto&quot;}

## Acessar a página de aterrissagem {#access-landing}

Depois de fazer logon na Adobe Experience Cloud e no Analytics, ative o botão [!UICONTROL Nova página de aterrissagem — Beta] no canto inferior esquerdo. O acesso ao botão de alternância é específico do usuário por organização, não específico da empresa.
...
* **[!UICONTROL Os relatórios]** se referem a tudo o que foi pré-criado pela Adobe, como relatórios no Reports &amp; Analytics e modelos no Workspace.
* **[!UICONTROL Os modelos]** não são mais usados como termo para projetos do Workspace pré-criados da Adobe. Agora eles estão em [!UICONTROL Relatórios]. O termo [!UICONTROL Modelos] O ainda é usado para modelos criados por sua empresa.

## Navegue até a guia [!UICONTROL Projetos] {#navigate-projects}

[!UICONTROL Os projetos] servem como página inicial do [!UICONTROL Workspace]. Qualquer projeto do Workspace é exibido aqui, incluindo scorecards para dispositivos móveis.
...
| Criar projeto | O menu suspenso permite criar um [!UICONTROL projeto do Workspace] ou um [!UICONTROL projeto móvel]. |
| Mostrar menos/mais | Alterna entre não mostrar e mostrar o banner: ![Banner superior](assets/top-banner.png) |
| Projeto em branco | Cria um [projeto do Workspace](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/home.html?lang=pt-BR) em branco para que você o preencha. |
| Cartão de pontuação para dispositivos móveis em branco | Cria um [cartão de pontuação móvel](https://experienceleague.adobe.com/docs/analytics/analyze/mobapp/curator.html?lang=pt-BR) em branco para que você o preencha. |
...
| Favoritos | Marcar um projeto como favorito coloca uma estrela junto a ele e o marca como um favorito que você pode filtrar. |
...
| Tipo | Indica se é um projeto do Workspace ou um cartão de pontuação móvel. |
...
| Aberto pela última vez | Quando este projeto foi aberto pela última vez por você. |
...
| &lt; (Botão voltar) | Esse botão em um projeto do Workspace ou em um relatório direciona você para a configuração mais recente da página de aterrissagem. Qualquer configuração de página que você tenha ao sair da página de aterrisagem persistirá ao retornar. |

## Clique na guia [!UICONTROL Relatórios] {#navigate-reports}

A guia [!UICONTROL Relatório] consolida três conjuntos de relatórios:

* Os modelos pré-criados do [!UICONTROL Workspace] que estavam anteriormente localizados em [!UICONTROL Workspace] > [!UICONTROL Projeto] > [!UICONTROL Novo]. A Adobe não usa mais a palavra &quot;modelo&quot; neste contexto.
* A maioria dos relatórios pré-criados no menu superior de [!UICONTROL Relatórios] do Adobe Analytics anterior. Esses relatórios agora são exibidos no [Analysis Workspace](https://experienceleague.adobe.com/docs/analytics/analyze/analysis-workspace/home.html?lang=en).
...
| Pergunta | Resposta |
| --- | --- |
| Onde estão os modelos que estou acostumado a ver no Workspace? | Esses modelos estão agrupados na guia [!UICONTROL Relatórios] |
...
| Meus favoritos atuais do Reports &amp; Analytics são transferidos? | Não, eles NÃO são transferidos. No entanto, todos os favoritos de Projeto do Workspace são transferidos. |
