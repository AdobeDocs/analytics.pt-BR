...
O Adobe alterou o separador entre dimensões e classificações de um caractere de sublinhado (_) para || Essa mudança tem implicações de compatibilidade para um usuário do Report Builder 5.0 que abre uma pasta de trabalho do Report Builder v5.1 com solicitações de classificação. Cada vez que uma pasta de trabalho de uma versão anterior à v5.1 for aberta, todas as suas solicitações de classificações serializadas serão convertidas para esse formato.
...
* Se você editar a solicitação, não alterar os parâmetros e, em seguida, clicar em Concluir, a resposta será expressa apropriadamente. Na verdade, editar a solicitação resolve o problema, pois os parâmetros do Layout de resposta são serializados novamente. Portanto, há uma solução alternativa, embora seja demorada.
...
* **Desempenho de solicitação individual**: a velocidade de processamento pode ser afetada pelas seguintes configurações:
  ...
  |--- |--- |--- |
  | Detalhamentos e ordem de detalhamento | Poucos | Muitos |
  ...
  | Intervalo de datas | Intervalo pequeno | Amplo alcance |
...
* **Horário de agendamento**: agendamento ordenado durante o período de 24 horas (consulte a tabela abaixo). Favoritos, painéis e pastas de trabalho do Report Builder já existentes que foram agendados muito próximos podem causar atrasos. Agende solicitações maiores e mais complexas no início da manhã para permitir que ocorram buscas manuais e atualizações durante o dia útil.
