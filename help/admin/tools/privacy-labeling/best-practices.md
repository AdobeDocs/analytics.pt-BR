...
Antes de descobrir quais rótulos devem ser aplicados aos campos/variáveis, primeiro é necessário compreender as IDs que você está capturando nos dados do Analytics e decidir quais serão usadas nas solicitações de Privacidade de dados. A Privacidade de dados expande o escopo do que pode ser considerado uma ID. As IDs estão em duas classes amplas: diretamente identificáveis (rótulo de identidade: I1) e indiretamente identificáveis (rótulo de identidade: I2).

* **Uma ID diretamente identificável (I1)**: nomeia a pessoa ou fornece um método direto para contatá-la. Os exemplos incluem o nome de alguém (até mesmo um nome comum, como John Smith, que pode ser compartilhado por centenas de pessoas), qualquer um de seus endereços de email ou números de telefone etc. Um endereço de correspondência sem um nome pode ser considerado diretamente identificável, mesmo que só possa identificar uma residência ou empresa em vez de uma pessoa específica dentro dessa residência ou empresa.
* **Uma ID indiretamente identificável (I2)**: não permite a identificação de um indivíduo por si só, mas pode ser combinada com outras informações (que podem ou não estar em seu poder) para identificar alguém. Exemplos de uma ID identificável indiretamente incluem um número de fidelização do cliente ou uma ID usada pelo sistema de CRM de uma empresa que seja única para cada um de seus clientes. Na Privacidade de dados, as IDs anônimas armazenadas nos cookies de rastreamento usados pelo Analytics podem ser consideradas como indiretamente identificáveis, embora possam identificar apenas um dispositivo e não uma pessoa. Em um dispositivo compartilhado, esses cookies não são capazes de distinguir os diferentes usuários do sistema. Por exemplo, apesar de o cookie não poder ser usado para localizar um computador que o contém, se alguém tiver acesso ao computador e localizar o cookie, esse indivíduo poderá vincular os dados de cookie do Analytics novamente ao computador.

Um endereço IP também é considerado como indiretamente identificável porque ele só pode ser atribuído a um único dispositivo em um determinado momento. No entanto, os ISPs podem e geralmente alteram os endereços IP da maioria dos usuários regularmente, de modo que, com o tempo, um endereço IP pode ter sido usado por qualquer um de seus usuários. Também não é incomum que muitos clientes de um ISP ou vários funcionários em uma empresa na mesma intranet compartilhem o mesmo endereço IP externo. Por causa disso, a Adobe não oferece suporte ao uso de um endereço IP como uma ID para uma solicitação de Privacidade de dados. Entretanto, quando uma ID que aceitamos for usada como parte de uma solicitação de exclusão, também apagaremos os endereços IP que ocorreram com essa ID. Você deve decidir se existem outras IDs coletadas que possam se enquadrar nessa categoria, de I1 ou I2, mas que não sejam adequadas para o uso como uma ID diferencial nas solicitações de Privacidade de dados.

Mesmo que sua empresa colete várias IDs diferentes nos dados do Analytics, é possível optar por usar apenas um subconjunto dessas IDs nas solicitações de Privacidade de dados. Os motivos para isso podem incluir:

* Em seus próprios sistemas, você pode mapear uma das IDs do (por exemplo, endereço de email) para uma ID diferente (como a ID do CRM). Então, por questões de consistência, você decide usar somente a ID do CRM nas solicitações de Privacidade de dados em seu processamento da Privacidade de dados.
* Você não tem um método para validar se alguém é realmente a pessoa associada à ID. Por exemplo, pode ser muito difícil validar se um endereço IP foi usado apenas por uma única pessoa e se a pessoa que envia a solicitação é realmente essa pessoa.
* Algumas IDs podem corresponder a várias pessoas e você não deseja correr o risco de retornar informações sobre uma pessoa para outra com a mesma ID. Por exemplo, mesmo que você possa verificar que o nome de alguém é John Smith, talvez você não queira retornar todos os dados sobre todos os John Smith do seu sistema.
* Outro exemplo é uma ID de dispositivo, como a ID de cookie do Analytics. Se a ID ocorrer em um aplicativo de celular, você poderá decidir que todas as interações que usam essa ID estejam disponíveis para o proprietário do telefone celular. No entanto, se ocorrer em um dispositivo compartilhado, como um computador doméstico ou um computador em uma biblioteca ou cybercafé, você poderá decidir que não consegue distinguir entre usuários desse dispositivo e o risco de retornar dados para um usuário diferente é muito grande para permitir o uso desse tipo de ID.

## Práticas recomendadas para IDs compatíveis com o Analytics {#best-practices-an}

Use esta tabela para determinar os tipos de IDs que serão usadas ao enviar solicitações de Privacidade de dados para o Analytics. Assim que você souber essas informações, será mais fácil determinar os outros rótulos que devem ser usados para as variáveis.
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Alguns clientes adicionam IDs em <a href="/help/implement/vars/page-vars/evar.md">variáveis de tráfego personalizadas (props) ou em variáveis de conversão personalizadas (eVars)</a>. Embora o mais comum seja um ID de CRM, outros incluem endereços de email, nomes de usuário para logon, números de fidelidade do cliente ou hashes desses valores. </p> 
    <ul id="ul_0B9492CF786046BB97E31CCF83A85FEA"> 
     ...
     <li id="li_8115B999E8DA46CAB359BCF1F4A4DCAE">Esses campos também exigem rótulos I1 ou I2 e devem incluir um rótulo DEL-PERSON ou DEL-DEVICE. Normalmente, a opção PESSOA/DISPOSITIVO do rótulo DEL corresponderá à opção PESSOA/DISPOSITIVO do rótulo ID. </li> 
    </ul> <p> É raro um conjunto de relatórios ter mais de uma ou duas variáveis personalizadas contendo IDs que você gostaria de usar para identificar os titulares dos dados nas solicitações de Privacidade de dados. Você pode ter várias variáveis com rótulos I1 ou I2 atribuídos, mas normalmente apenas uma ou duas delas também têm rótulos ID-PERSON ou ID-DEVICE. </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p>ID de visitante personalizada </p> </td> 
   <td colname="col2"> <p>Embora isso não seja amplamente usado, o Analytics também oferece suporte a uma implementação em que uma ID de visitante personalizada pode ser fornecida, que, se presente, será usada no lugar do cookie herdado de rastreamento do Analytics. Esse campo tem os rótulos I2, ID-PERSON e DEL-PERSON. </p> <p>Muitas implementações derivam essa ID de uma ID de CRM, para que ela esteja presente somente enquanto alguém estiver conectado ao site. Isso permite que uma mesma ID de visitante personalizada seja usada em vários dispositivos. Uma desvantagem técnica é que o rastreamento que acontece antes do logon do usuário não pode ser vinculado ao rastreamento coletado após o logon. Se, em vez disso, você usar a ID de visitante personalizada para simplesmente identificar um dispositivo, deverá alterar os rótulos ID-PERSON e DEL- PERSON para ID-DEVICE e DEL- DEVICE, respectivamente. </p> </td> 
  </tr> 
 </tbody> 
</table>
...
* Os rótulos DEL resultarão na [anonimização](/help/admin/tools/privacy-labeling/labels.md#data-governance-labels) dessas variáveis (a ID será substituída por uma sequência de caracteres aleatória com o prefixo “Privacidade de dados-”). O mesmo valor anônimo substituirá todas as instâncias do valor original em todas as ocorrências que tenham sido identificadas por uma ID usada na solicitação. Se o valor original neste campo era uma dessas IDs, as métricas de relatório não serão alteradas.
