...
![eVars Merch](/help/admin/tools/manage-rs/edit-settings/conversion-var-admin/assets/merch-evars1.png)
...
| Habilitar o merchandising | Um sinalizador que precisa ser definido como &quot;Habilitado&quot; para transformar o eVar de um eVar padrão em um eVar de comercialização |

### Habilitar o merchandising

Quando a configuração &quot;Habilitar merchandising&quot; estiver definida como &quot;Habilitado&quot;, todas as configurações conforme descrito abaixo serão exibidas no Gerenciador de conjunto de relatórios. Quando a configuração &quot;Habilitar merchandising&quot; estiver definida como &quot;Desabilitado&quot;, somente as configurações de eVar padrão estarão disponíveis.
...
* [!UICONTROL Eventos] são úteis para registrar valores de evento de moeda ou incrementais personalizados que não devem ser contados como receita (como envio, descontos etc.)
...
Por exemplo, muitos produtos de roupas têm &quot;SKUs filhos&quot;, que designam o tamanho, a cor, o estilo e quaisquer outros atributos. Esses atributos separam um único produto filho de outros produtos que pertencem ao mesmo produto pai. Digamos que você decida comprar uma camiseta azul tamanho médio e uma camiseta vermelha tamanho grande. Suponha que ambas as camisetas tenham a ID de produto pai &quot;tshirt123&quot; e o `eVar10` tenha sido configurado para capturar SKUs filhos. As variáveis definidas na página de confirmação de compra seriam definidas da seguinte maneira:
...
A lógica adicional contida no arquivo AppMeasurement/Adobe Experience Platform Web SDK pode preencher o restante das variáveis (eVars/dimensões de merchandising) que precisam ser definidas ao mesmo tempo.\
...
* Merchandising habilitado: Habilitado
...
Em resumo, sem configuração adicional, a métrica Instâncias pronta para uso de uma eVar de merchandising é menos que útil. Felizmente, o Adobe lançou [Atribuição](/help/analyze/analysis-workspace/attribution/overview.md). Ele permite aplicar vários modelos de atribuição a qualquer métrica personalizada que o Adobe Analytics coleta. As métricas que aplicam esses modelos de atribuição não usam os valores contidos nas colunas post_evar ou os valores que estão vinculados a um produto específico. Em vez disso, essas métricas usam apenas os valores que são transmitidos por meio das próprias solicitações de imagem (ou valores capturados por meio das regras de processamento do Adobe Analytics). Você pode usar os recursos na Atribuição para obter uma métrica de instâncias atribuída com precisão para todas as eVars de merchandising que usam a Sintaxe de variável de conversão.

![Seleção de atribuição](/help/admin/tools/manage-rs/edit-settings/conversion-var-admin/assets/attribution-select.png)

Ao adicionar uma métrica de instância para um eVar de merchandising a um relatório, o modelo de Atribuição adequado seria o modelo &quot;Último contato&quot;. Nesse caso, a configuração da Janela de pesquisa do modelo não importa. O motivo é que um modelo de atribuição de Último contato &quot;forçado&quot; sempre dá crédito de instância para cada valor individual transmitido por uma solicitação. Isso ocorre independentemente de as configurações de atribuição/vínculo reais da eVar serem definidas como &quot;Mais recente (último)&quot; a &quot;Valor original (primeiro)&quot;.
