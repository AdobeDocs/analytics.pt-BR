...
description: Use classificações para categorizar valores em grupos relacionados e para relatar no nível do grupo. Por exemplo, você pode classificar todas as campanhas de Pesquisa paga em uma categoria como Cartões de vídeo. Em seguida, você pode relatar o sucesso dessa categoria com relação a métricas como Instâncias (ou click-throughs).
...
| Termo | Definição |
|--- |--- |
| Classificações de texto | Especifica o tipo de categoria de classificação que permite agrupar dados variáveis para fins de relatório. |
| Nome | O nome da classificação. A classificação adicionada aparece como opção de Detalhamento de Canal no Gerenciador de canal de marketing. |
| Opções | O nome da classificação. A classificação adicionada aparece como opção de Detalhamento de Canal no Gerenciador de canal de marketing. |
