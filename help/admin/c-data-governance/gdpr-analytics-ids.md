...
seo-title: Práticas recomendadas de rotulagem
...
* **Uma ID indiretamente identificável (I2)**: não permite a identificação de um indivíduo por si só, mas pode ser combinada com outras informações (que podem ou não estar em sua posse) para identificar alguém. Exemplos incluiriam um número de fidelidade do cliente ou uma ID usada pelo sistema de CRM de uma empresa, exclusiva para cada um de seus clientes. Em Privacidade de dados, as IDs anônimas armazenadas nos cookies de rastreamento usados pelo Analytics podem ser consideradas como identificando indiretamente, mesmo que apenas possam identificar um dispositivo em vez de um indivíduo; em um dispositivo compartilhado, esses cookies não podem distinguir entre usuários diferentes do sistema. Por exemplo, apesar de o cookie não poder ser usado para localizar um computador que o contém, se alguém tiver acesso ao computador e localizar o cookie, esse indivíduo poderá vincular os dados de cookie do Analytics novamente ao computador.

   Um endereço IP também é considerado indiretamente identificável porque só pode ser atribuído a um único dispositivo em um determinado momento. No entanto, os ISPs podem mudar, e frequentemente mudam, os endereços IP para a maioria dos usuários; assim, ao longo do tempo, um endereço IP pode ter sido usado por qualquer um dos seus usuários. Também não é incomum que muitos clientes de um ISP ou vários funcionários de uma empresa que usem uma intranet compartilhem o mesmo endereço IP externo. Because of this, Adobe will not support using an IP address as the ID for a [Data Privacy request.](../../admin/c-data-governance/gdpr-submit-access-delete.md#submit-requests) Entretanto, quando uma ID que aceitamos for usada como parte de uma solicitação de exclusão, também apagaremos os endereços IP que ocorreram com essa ID. Você deve decidir se existem outras IDs coletadas que podem estar incluídas nessa categoria, de I1 ou I2, mas não são adequadas para uso como uma ID distinta para solicitações de Privacidade de dados.

Mesmo se sua empresa coletar várias IDs diferentes nos dados do Analytics, você pode optar por usar apenas um subconjunto dessas IDs para solicitações de Privacidade de dados. Os motivos disso podem incluir:

* Nos seus próprios sistemas, é possível mapear uma das IDs (por exemplo, endereço de email) a uma ID diferente (como a ID do CRM). Em seguida, para fins de consistência, você decide usar somente a ID do CRM para solicitações de Privacidade de dados no processamento de Privacidade de dados.
...
Use esta tabela para determinar os tipos de IDs que serão usadas ao enviar solicitações de Privacidade de dados ao Analytics. Quando souber essas informações, será mais fácil determinar os outros rótulos que deverão ser usados para as variáveis.
  ...
  <tr> 
   <td colname="col1"> <p>IDs de cookies </p> 
    <ul id="ul_CB43CEA3054E490585CBF3AB46F95B5B"> 
     ...
     <li id="li_7B6A9A788BBD47428315B3893FC07BC3"> <a href="https://marketing.adobe.com/resources/help/en_US/mcvid/" format="https" scope="external"> Cookie do serviço de identidade </a> (ECID), anteriormente conhecido como Marketing Cloud ID (MCID) </li> 
    </ul> </td> 
   <td colname="col2"> <p>Esses cookies identificam um dispositivo ou, mais especificamente, um navegador para um usuário de um dispositivo. Em um dispositivo compartilhado no qual um logon comum é usado, essa ID pode se aplicar a qualquer/todos os usuários do dispositivo. Adobe has created some <a href="https://www.adobe.io/apis/cloudplatform/gdpr/services/allservices.htm" format="html" scope="external"> unified JavaScript </a> that you can place on your website to collect these cookies if you want to allow them to be used for Data Privacy requests. </p> <p>Os usuários do SDK móvel do Adobe Analytics também têm uma Experience Cloud ID (ECID). Há chamadas de API no SDK para ler essa ID, de modo que você pode aprimorar seu aplicativo para coletá-la para uma solicitação de privacidade de dados. </p> <p>Muitas empresas consideram as IDs de cookies do navegador como IDs de dispositivos compartilhados. Como resultado, em consulta com as equipes legais, eles podem optar por não oferecer suporte para uso como IDs aceitáveis para solicitações de Privacidade de Dados ou podem optar por retornar apenas uma quantidade muito limitada de dados quando essas IDs forem usadas ou apenas aceitá-las para solicitações de exclusão. </p> <p>Esses cookies têm um rótulo ID-DEVICE que não pode ser alterado (assim como os rótulos I2 e DEL-DEVICE). A configuração padrão do Adobe Analytics retornará apenas informações genéricas sobre o dispositivo, como o tipo de dispositivo, o sistema operacional, o navegador, entre outros, bem como a hora/data em que o site foi visitado ao usar essas IDs. No entanto, se você optar por oferecer suporte a essas IDs para solicitações de Privacidade de dados, conforme discutido abaixo, poderá adicionar ou remover rótulos ACC-ALL para configurar o conjunto exato de campos que deseja que sejam retornados para uma solicitação de acesso de Privacidade de dados. </p> <p>Principalmente se o conjunto de relatórios corresponder a um aplicativo para dispositivos móveis que exige logon, você pode decidir que a ID da Experience Cloud do dispositivo corresponde a um usuário específico e, por isso, desejará rotular mais campos com o rótulo de ACC-ALL, inclusive os nomes das páginas visitadas, os produtos visualizados, entre outros. </p> <p>Observação:  Se você especificar a opção "ExpandirIds" na solicitação de Privacidade de dados, suas solicitações sempre incluirão IDs de cookie, além de quaisquer outras IDs especificadas. Consulte <a href="../../admin/c-data-governance/gdpr-id-expansion.md" format="dita" scope="local">Expansão de ID</a> para obter mais detalhes. Nesses casos, as ocorrências com apenas uma ID de cookies, mas não outra ID, retornarão apenas dados rotulados como ACC-ALL como parte da solicitação de acesso. </p> </td> 
  </tr> 
  <tr> 
   ...
   <td colname="col2"> <p>Alguns clientes adicionam IDs em <a href="https://marketing.adobe.com/resources/help/en_US/sc/implement/props_eVars.html" format="html" scope="external">variáveis de tráfego personalizadas (props) ou em variáveis de conversão personalizadas (eVars) </a>. Embora o mais comum seja um ID de CRM, outros incluem endereços de email, nomes de usuário para logon, números de fidelidade do cliente ou hashes desses valores. </p> 
    <ul id="ul_0B9492CF786046BB97E31CCF83A85FEA"> 
     <li id="li_D35B61CC6A8B485A8E09358A46D3F598">Se quiser usar uma dessas IDs para solicitações de Privacidade de dados, forneça ao campo que contém uma etiqueta ID-PESSOA. </li> 
    ...
    </ul> <p> É raro que um conjunto de relatórios tenha mais de uma ou duas variáveis personalizadas contendo IDs que você deseja usar para identificar as pessoas de dados para solicitações de Privacidade de dados. É possível ter várias variáveis com os rótulos I1 ou I2, mas tipicamente, apenas uma ou duas delas também terão os rótulos ID-PERSON ou ID-DEVICE. </p> </td> 
  </tr> 
  <tr> 
...
>[!NOTE]
>
>Props sempre diferenciam maiúsculas de minúsculas. As eVars não diferenciam maiúsculas de minúsculas por padrão, mas podem ser configuradas pelo Atendimento ao cliente da Adobe para fazer essa diferenciação. Se você tiver uma eVar com distinção entre maiúsculas e minúsculas que contenha uma ID, é sua responsabilidade usar as letras maiúsculas e minúsculas corretas ao enviar uma solicitação de privacidade de dados para que as letras maiúsculas e minúsculas usadas na solicitação correspondam às letras maiúsculas e minúsculas usadas nas ocorrências que contêm essas IDs.

Os rótulos de exclusão DEL-DEVICE e DEL-PERSON devem ser usados com moderação. Quando aplicado a uma variável que não contém uma ID que foi usada como parte da solicitação de Privacidade de dados, as contagens (métricas) nos relatórios históricos do Analytics quase sempre serão alteradas.
...
* The DEL-labels will result in these variables being [anonymized](../../admin/c-data-governance/gdpr-labels.md#section_F3DEE591671A4B16A8E043F91C137ECB) (the ID will be replaced with a random string prefixed with "Data Privacy-"). O mesmo valor anonimizado substituirá todas as instâncias do valor original em todas as ocorrências que foram identificadas por uma ID usada na solicitação. Se o valor original desse campo for uma dessas IDs, as métricas de relatório não serão alteradas.
...
* Se um campo não tiver um rótulo de ID, mas contiver informações de identificação que você deseja anonimizar, o rótulo apropriado (DEVICE ou PERSON) dependerá da sua implementação. Se você usar somente IDs de cookie para solicitações de Privacidade de dados, você deverá usar DEL-DEVICE.
...
* Observe que, se um rótulo de DEL-DEVICE ou DEL-PERSON for especificado em qualquer variável que também não seja usada como uma ID para essa solicitação (incluindo uma ID expandida), os valores exclusivos nessa variável serão anonimizados apenas em ocorrências que tiverem uma ID especificada (ou expandida). Se outras ocorrências contiverem o mesmo valor, elas não serão atualizadas nesses outros locais. Isso pode resultar em alterações de contagens (métricas).
   ...
   Por exemplo, se você tiver três ocorrências contendo o valor "foo" na eVar7, mas apenas uma delas também contiver uma ID em uma variável diferente que é correspondida para uma exclusão, então "foo" nessa ocorrência será modificado para um valor como "Privacidade de dados-123456789", enquanto permanecerá inalterado nas outras duas ocorrências. Um relatório que mostra o número de valores exclusivos para eVar7, agora, também mostrará mais um valor exclusivo. Um relatório que mostra os principais valores para eVars pode incluir “foo” com apenas duas instâncias (em vez de três, como anteriormente), e o novo valor também será exibido com uma única instância.

## Práticas recomendadas para definir rótulos de acesso {#section_AC7E216F81C141FCA6A62F8836E06EE7}

Embora pouquíssimos campos tenham um dos outros rótulos, é comum que um grande número de campos tenha rótulos ACC. As etiquetas de acesso apropriadas dependerão das IDs que você está usando para solicitações de Privacidade de dados.
