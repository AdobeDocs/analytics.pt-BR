...
## Dados de ocorrência de amostra

Suponha que você tenha os seguintes dados de ocorrência:

* A primeira linha contém os rótulos para cada variável.
* A segunda linha é o nome da variável. Se ele tiver um rótulo de ID, ele conterá a namespace atribuída entre parênteses.
* start de dados de ocorrência na terceira linha.
...
| Dados de ocorrência | Mary | 77 | A | Seg | X |
|  | Mary | 88 | B | N | S |
|  | Mary | 99 | C | O | Z |
|  | John | 77 | D | P | Qua |
|  | John | 88 | E | N | U |
...
| user=Mary | true | dispositivo | not present | 77, 88 | not present | N, P | U, W |
| user=Mary AAID=66 | true | pessoa | Mary | 77, 88, 99 | A, B, C | M, N, O | X, Y, Z |
| user=Mary AAID=66 | true | dispositivo | not present | 66, 77, 88 | not present | N, P | U, W, Z |
| xyz=X | false | dispositivo | not present | 55, 77 | not present | M, R | X |
| xyz=X | true | dispositivo | not present | 55, 77 | not present | M, P, R | W, X |

Observe que a configuração para expandeIDs não faz diferença na saída quando uma ID de cookie é usada.

## Exemplo de solicitação de exclusão

Com uma solicitação de exclusão usando os valores da API na primeira linha da tabela, a tabela de ocorrência será atualizada para se parecer com o seguinte:
