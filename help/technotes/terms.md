...
* **Componente:** os componentes do Analysis Workspace consistem em dimensões, métricas, segmentos, intervalos de datas, alertas e métricas calculadas que você pode arrastar e soltar em um projeto. Consulte [Visão geral de componentes](/help/analyze/analysis-workspace/components/analysis-workspace-components.md) no Guia de ferramentas do Analytics.
...
* **s<span>.</span>tl():** o nome da função em uma biblioteca AppMeasurement que envia uma solicitação de imagem de rastreamento de link. Algumas bibliotecas do AppMeasurement usam `s.trackLink()`. Consulte [tl](../implement/vars/functions/tl-method.md) no guia de usuário Implementar.
